...
function setupSocketHandlers(socket, io) {
  ...
  socket.on('test_connection', (data) => {
    ...
    console.log(`🔌 Test connection received from meeting ${meetingId}`);
  ...
  // Force end meeting (host ends call for everyone)
  socket.on('force_end_meeting', (data) => {
    const { meetingId, reason } = data;
    console.log(`🚨 Host force ending meeting ${meetingId}: ${reason}`);
    console.log(`🚨 Broadcasting to room: ${meetingId}`);
    console.log(`🚨 Room members:`, Array.from(io.sockets.adapter.rooms.get(meetingId) || []));
    
    io.to(meetingId).emit('force_end_meeting', {
      meetingId,
      reason,
      timestamp: new Date()
    });
    console.log(`✅ Force end meeting signal sent to all participants in ${meetingId}`);
  });
}
